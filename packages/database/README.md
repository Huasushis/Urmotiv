...
`0009_robot_review_leases` 需要维护窗口。先停止 Fermata 的新轮询，保持旧版 Urmotiv API 可用，让已经开始的
付费请求持续流式读取到服务端真正结束并提交完成结果；不能取消或用 120 秒总时限截断。确认所有租约已经完成或
自然过期后，完整停止旧版 Urmotiv API，并等待本项目的数据库事务排空，再运行迁移。

只要仍有未过期或没有到期时间的机器人租约，迁移就会固定失败；任何涉及表仍被占用时也会立即安全失败，需要排空后
重试。迁移用不等待的独占锁保护全部涉及表，避免在旧、新两种相反的加锁顺序之间形成死锁。过期租约会归档为
`expired`，旧撤销记录会归档为 `legacy_closed`，人工审核意见保持不变。

题目包任务 outbox 按四个兼容阶段上线。outbox 是先把“需要投递到 Redis 的任务”作为数据库事实保存，再由独立
进程可靠投递的表；它只保存任务编号、队列身份、请求摘要、租约和固定错误码，不保存题面、题解、文件内容、
存储路径、Redis 原始错误或其他私有材料。

1. **C1，兼容建表。** `0010_problem_package_job_outbox` 只增加空表、表内检查、索引、不可变规则、执行栅栏和
   删除或清空防护。它不回填旧任务，不修改 `import_jobs`、`export_jobs`，也不增加要求两边必须同时存在的
   外键或延后触发器。旧版 API 在迁移后仍可照常创建没有 outbox 的 `queued` 任务，旧 worker 也可继续处理已有
   任务。这个阶段不要求停服或排空任务；PostgreSQL 执行建表时仍会取得自身需要的短时数据库锁，部署脚本应在
   锁等待超时后安全重试，不能把一次短暂冲突变成破坏性恢复。0010 还没有接入 API、Redis 或 worker。
2. **C2，休眠接入。** 独立提交加入事务内写 outbox 的仓储、可信回填、投递器和 worker 栅栏，但先用配置保持
   休眠。回填必须从数据库中固定的任务快照重建 `@urmotiv/jobs` 请求，再用 `digestJobRequest` 计算摘要；缺少
   必要事实时暂停并交给人工处理，不能由 SQL 猜造。此时需要证明旧任务没有 outbox 仍能工作，新代码关闭时也
   不改变旧流程。
3. **C3，维护窗口切换。** 停止接收新的题目包任务，让旧 worker 排空，逐行回填并核对，再使用新的 Redis
   命名空间启用 API、投递器和 worker。API、worker、部署配置必须一起切换；停止宽限时间要长于允许的最长任务，
   不能用短超时取消仍正常执行的任务。切换时任何数量不一致、未知 Redis 状态、取消结果或协议冲突都要暂停并
   人工恢复，不能把 Redis 的终态直接写成数据库 `failed`。
4. **C4，关系收紧。** 完成切换后先盘点每个仍活跃的导入、导出任务及其 outbox，验证无缺失、重名和状态冲突，
   再由后续迁移增加父表关系、延后触发器等跨表强约束。C4 不能倒填进 0010，否则尚未切换的旧 API 会在创建
   `queued` 任务时立即失败。

业务任务编号 `job_id` 在整个恢复过程中不变；实际交给 Redis 的 `queue_job_id` 是独立 UUID。每条 outbox 从
`delivery_generation = 1` 开始，并保存明确的 `max_delivery_generations` 上限。`queue_job_ids` 按代数保存这项业务任务
使用过的全部队列编号：初始只有第一代编号，之后只能在原数组末尾追加一个从未使用的新编号，不能删除、替换、重排
或复用较早代的编号；outbox 行本身也禁止删除和清空。新建第一代身份和轮换身份时还会取得专用的事务级数据库锁，
再检查全部 outbox 的完整编号历史，防止另一个业务任务复用已经离开当前列的旧编号。这个二元锁使用
`(1431453002, 1651666805)`，与首管理员初始化锁不同；所有后续身份写入也必须沿用同一顺序，不能改成无锁查询。
因此并发请求中也至多一个请求能登记同一编号，即使第三代以后再次恢复，也不能重新命中 Redis 中另一项或第一代已经
终结的任务。同一代的当前队列编号固定，请求摘要在全部代之间都固定不变。

身份登记只允许使用 PostgreSQL 默认的 `READ COMMITTED`（读已提交）事务隔离级别。在这个级别中，事务取得上述锁后，
触发器里的动态数据库扫描能看到刚刚提交的完整编号历史。`REPEATABLE READ`（可重复读）和 `SERIALIZABLE`（可串行化）
会保留更早的读取快照，等待锁后仍可能看不到另一事务刚提交的历史，因此新建 outbox 或增加投递代数会固定失败，不能
降级为无锁检查；生产身份登记必须使用默认级别。续租、记录固定错误码等不改变队列身份的更新不受这个限制。

只有确认上一代已经投递、没有活跃投递租约或执行租约，且数据库业务任务仍可恢复时，才可把代数恰好加一、追加新的
`queue_job_id` 并重新投递；达到上限后必须暂停并人工处理。Redis 的成功、失败、取消或缺失状态都不能自动映射为
数据库结论。取得执行租约前还必须有当前代成功投递的明确证据，也就是 `last_dispatched_at` 已写入且
`next_dispatch_at` 已清空；只创建 outbox 或刚安排重投时不能开始执行。

`execution_fence` 是每次取得新执行租约时恰好加一的数字。投递租约和执行租约都同时绑定当前
`delivery_generation` 与 `queue_job_id`；worker 后续写入进度或结果时还必须匹配业务任务编号、队列租约编号、
当前栅栏、未过期时间和 `retired_at IS NULL`。匹配不到就是旧投递器或旧执行者，必须停止，不能覆盖新一代的结果。
相同栅栏下不得替换队列编号、租约编号、worker、尝试次数或领取时间，过期租约也不能通过续租恢复。退役时不能有
仍有效的执行租约；已经过期的执行租约必须在设置 `retired_at` 的同一条原子更新中清空全部执行身份。退役后的行完全
不可修改，旧 worker 即使仍持有原来的编号也不能再写业务进度或结果。
