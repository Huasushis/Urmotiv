...
- **环境变量输入**：CLI 只接收 `*-env` 参数，没有数量参数；包数量由权威批准候选与包扫描结果共同决定。
  私有根、包目录、元数据、分组、物化目录、候选目录、批准文件、Phase 1 回执、回执目录、存储根、导入
  输出、管理连接、验收库名、执行主体、标签、提交、批次摘要、来源绑定摘要、Phase 1 回执摘要、执行编号
  和目标分类都必须来自命名环境变量。私有根固定为项目内、所有者独占且 Git 忽略的
  `private/phase2-evidence`，所有输入都必须在其中。
- **变更前完整校验**：重新验证物化源、候选与人工批准，再把批准候选重算出的逐包身份、ZIP 中全部修订
  字段的完整内容清单和包摘要绑定进回执；重新读取 Phase 1 回执和完成标记；核对批准摘要、代码清单提交、
  主体/标签/执行编号绑定；只读验证来源数据库十张表、标签、执行主体和目标库不存在。任一失败均不创建
  验收库、导入输出或 runner 回执。
- **快照与恢复**：新建验收库完成迁移和种子后关闭全部连接，再用 PostgreSQL `TEMPLATE` 创建库快照；
  本地存储快照按文件路径、字节数和内容摘要校验。失败恢复先建立 staging 库并验证，再原子替换目标；
  存储也用 staging/backup 原子切换。恢复进入时先写 `PHASE2_RECOVERY_IN_PROGRESS`；数据库与存储都恢复
  到精确基线并复核后才删除快照和恢复标记。数据库或存储任一恢复失败、快照内容被改或清理失败都会保留
  现场并向上返回，不能写通过标记。
...
- **精确增量**：逐表核对当前完整十表集合：用户、题目、修订、修订标签、修订附件、样例、导入任务、
  ...
  从数据库结果反推；缺失题解和附件只按本次清单的问题标识统计。缺失（NULL）题解与实际为空的题解分开
  统计，两者都不得与批准候选的计数发生漂移。全生命周期修订内容清单在扫描、首遍、编辑后与重放后各复算
  一次，冻结字段必须全程不变。
...
- **验收门**：真实 PostgreSQL 双场景验收固定用 Node 24 运行
  `test:phase2-postgres:acceptance`。合成 137 包场景必须证明 137 首遍导入、137 幂等解析重放、修订完整
  一致性；故障场景必须证明首遍后故障精确回滚、数据库快照被改被拒、清理失败被拒。正式验收不挂测试钩子，
  由 runner 自己验证已提交的代码清单。
