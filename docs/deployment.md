...
Urmotiv 中的 Anklang 插件只接受 `http://127.0.0.1:8730`、`localhost`、`host.docker.internal`、RFC1918/链路本地/ULA 字面量或单标签容器服务名；不得填写公网地址、账号密码、路径、查询或片段。管理员必须在确认 Anklang 自己的数据库、对象存储和所选外部来源/embedding 链路全程留在批准私有边界后，才把 `privateContentAuthorized` 从默认 `false` 改为 `true`。检索来源可选 `yuantiji`（默认，仅公开检索）、`local` 或 `hybrid`；两个插件密钥分开保存：`serviceToken` 认证 Urmotiv→Anklang 服务请求，`embeddingApiKey` 是 local/hybrid 所需的 OpenAI 兼容嵌入提供方写密钥；`embeddingProvider`（`protocol: openai`、`baseUrl`/`model`/`dimension`）是普通设置，地址只允许 HTTPS 或仅供隔离测试的本地/私有 HTTP。保存后可在插件卡片运行连接测试；local/hybrid 更换提供方会由 Anklang 分批全量重建向量。`serviceToken` 使用插件密钥保存，不会出现在 `/api/v1/admin/plugins` 响应。一个完整的设置请求示例见 `plugins/anklang/README.md`。
