...
2. `parse-metadata.py` 解析表格为 `metadata.json`（自动剔除 QQ 号等隐私、丢弃审核人列，难度取区间中点
   到整百）。
3. `apps/api/scripts/migrate-hist.ts` 用便宜模型逐题规范化，产出预览 JSON 和原生包；多题合一的文件会
   自动拆分。**每一题都要人工预览确认**后再导入，模型可能漏读或误判。
4. 导入账号：原生包不含作者身份，出题人学号保留在包的迁移备注里。如果批量为出题人建账号麻烦、或担心
   他们首次 CAS 登录出问题，可以**先建一个共享的“历史导入”账号**（用有 `problem.import` 权限的账号），
   用它统一导入全部历史题目；日后再由管理员按学号把题目归属逐步转移给本人账号。这样迁移不必等所有出题人
   都完成统一身份首登。
