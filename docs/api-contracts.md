...
| `PUT /api/v1/problems/:problemId/external-review` | 真人账号有本题 `problem.status.change` 时设置外部验题；请求为 `{ "enabled": false, "expectedRevision": 3 }`，返回更新后的题目；版本冲突返回 409，不可见题目返回 404 |
