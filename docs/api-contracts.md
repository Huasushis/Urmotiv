...
`POST /api/v1/me/password/initialize` 接收 `{newPassword}`，仅供尚无密码的人工账号在近期直接登录后初始化密码；root、模拟会话、已存在密码或未验证联系邮箱均不能走此入口。近期依据服务端会话创建时间，窗口为 10 分钟。响应中的 `canInitializePassword` 只用于提示，服务端提交时重新核对状态。

### 关联身份

`GET /api/v1/me/identities` 返回本人的关联记录与当前入口能力，数据只包括提供方、gid/id、认证学号、认证姓名、学校邮箱和上次认证时间，不返回票据或令牌。`POST /api/v1/me/identities/ustc/start` 接收 `{currentPassword}`，返回授权地址并设置浏览器绑定 Cookie；回调共用版本化 USTC 路径，但签名状态明确绑定本地账号、会话摘要和认证版本。

`POST /api/v1/me/identities/:provider/unlink` 接收 `{subject,currentPassword}`，只解绑指定提供方与身份编号。操作只允许本人直接登录的人工账号，拒绝 root、机器人和模拟会话；必须保留已启用的本地登录或其他已绑定、已启用的登录方式，失败返回 `LAST_LOGIN_METHOD`。绑定/解绑与认证版本变更、会话撤销及审计在同一事务完成。外部登录建立会话时再次检查关联仍然存在，解绑后的旧回调不能直接恢复原账号会话。

USTC 登录以提供方 + gid/id 找账号；学校提供的用户名、姓名或邮箱是资料，不是匹配已有账号的依据。自动建号仍由后台开关控制，冲突或关闭时引导登录后主动绑定。资料缺少邮箱或姓名不改变身份唯一性；未明确发布 `email_verified:true` 的新邮箱不会被自动视为已验证。已有账号可通过 `POST /api/v1/me/email-verification` 验证联系邮箱，不受公开注册开关限制。
...
| `PUT /api/v1/problems/:problemId/external-review` | 真人作者有本题编辑权，或真人管理员有本题 `problem.status.change` 时设置 AI/外部验题；请求为 `{ "enabled": false, "expectedRevision": 3 }`，返回更新后的题目；版本冲突返回 409，不可见题目返回 404；机器人不能更改此开关 |
...
`GET /api/v1/transfer/formats` 返回当前可用格式的 `items: [{ id, displayName }]`，需要登录。停用格式不会出现在列表中，任务执行仍复查绑定的格式版本。

导出预览与创建接口可附带 `contest: { id, expectedUpdatedAt }`，表示导出整个比赛。`problems` 必须包含方案全部题目且顺序一致，服务器自行选择方案固定的修订，客户端不能替换成最新版本；预览返回真实固定 `revisionId`，创建时可带回以检查一致性。方案已更新或缺题/换序返回 409，无比赛导出权限或不存在均返回 404。任务状态和下载继续使用原有导出接口，并复查比赛与题目权限。

当前格式编号由内置插件提供：`hydro`、`fps`。上传、预览、创建任务和下载都再次执行权限/安全检查；任务响应只返回编号、状态、计数和提示，不返回题面、题解或文件正文。摘要和 UUID 应使用上传/预览响应实际返回的值，不要在客户端伪造。

## 管理和机器人路由

`DELETE /api/v1/admin/users/:userId` 接收 `{ "confirm": true }`。要求真人直接登录并具有 `user.delete`、`problem.edit.all`、`contest.edit.all`；不能删除自己、root 或机器人。服务器在同一事务内复查权限，将题目和比赛归属转给调用者、撤销目标登录和角色，并追加审计。对任何目标内容的明确拒绝或审计失败都会使整个操作回滚。账号记录只保留作历史引用，删除后不再出现在账号目录或允许登录。
