...
两种格式都支持一次导入多题；多题包只能创建新题，不能共同覆盖一个题目编号。当前格式插件默认可以处于停用状态；导入/导出前，在 `/admin` 确认相应插件已启用且版本与任务绑定一致。任务排队后插件停用、升级或清单改变，会以固定的“格式当前不可用”失败，不会静默换用另一份实现。导入时保留的格式扩展会随后续题目修订一起保存，普通编辑不会清除它们。
...
比赛页面复用同一导出流程：服务器根据方案解析全部题目的固定版本，拒绝缺题、换序、过期方案和无权请求。比赛整包内的 ZIP 格式按 `001/`、`002/` 题目目录排列，XML 格式按 `001-…xml` 文件排列。原始单题包先通过安全检查，再增加顺序目录，不能借目录前缀掩盖路径跳转。仍受整个包的大小和文件数量限制。
