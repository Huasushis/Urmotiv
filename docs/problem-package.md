...
当前导入实现会把整个 ZIP 和解压结果放在内存中处理，因此默认限制为：原始 ZIP 不超过
128 MiB（134,217,728 字节），包内单个文件不超过 128 MiB，全部文件解压后的总量不超过
...
处理过程中可能同时保留原始 ZIP、解压内容和检查时使用的副本，实际占用会明显高于 128 MiB，
...
选择一道题时，下载内容就是该题的目标格式 ZIP。选择多道题时，系统先按相同的安全规则分别生成单题包，
再把这些单题包放进一个多题导出外层包；只有这一层允许包含 ZIP，单题包内仍禁止嵌套压缩包。
