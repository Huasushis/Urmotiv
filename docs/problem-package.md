...
导入幂等性以来源绑定摘要（`sourceBindingSha256`）为身份，不依赖标题或包字节摘要。
来源绑定取标题无关的五元组：来源编号、来源内容摘要、源路径、源内容映射摘要、元数据题号。
不包含标题/名称、元数据文件摘要、来源映射摘要（`sourceMappingSha256`）、题面、题解或包字节。
因此仅修改元数据中的标题（`name`）不会改变导入身份：元数据文件摘要和来源映射摘要会变，
但来源绑定不变。回放同一来源绑定时解析到既有题目编号，不会创建重复题目，也不会覆盖后来授权修改的标题。
不同来源绑定即使标题相同也保持为不同题目。
