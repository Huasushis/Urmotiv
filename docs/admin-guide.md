...
“系统管理员”负责账号、权限、运行设置和插件，也可以管理题目状态；“组长”负责日常题目终审和组题。两者可以由同一名人工用户兼任，明确拒绝的权限仍然优先。

## 首次管理员与恢复

### 开启邮箱注册链接验证

1. 打开「管理 → 系统」（`/admin/settings`），确认公开站点 URL 是用户实际访问的地址。验证邮件中的链接会使用这个地址。
2. 填写 SMTP 主机、端口、发件邮箱和服务商提供的用户名、密码或授权码。465 端口通常勾选「连接后立即使用 TLS」；587 端口会自动升级为加密连接。
3. 开启「允许用户名或邮箱登录」和「允许访客通过邮箱注册账号」，保存常规设置。缺少发信配置时，系统会拒绝开启注册。
4. 在退出登录的浏览器中点击「注册新账号」，填写邮箱、昵称和至少 12 个字符的密码。打开邮件中的链接，点击「确认邮箱」，再用邮箱和密码登录。未验证邮箱不能登录；链接只能使用一次，重新发送会使旧链接失效。

注册与统一身份登录可以分别控制。暂不使用统一身份认证时，在「管理 → 统一身份认证」（`/admin/oauth`）关闭启用开关；无需删除原客户端配置。关闭邮箱注册会停止新注册与匿名重发，不会停用已有账号；已发出且未过期的链接仍可验证。已有账号本人验证联系邮箱不受公开注册开关限制。

「配置完整」只表示 SMTP 必填项齐全，不代表邮件已经送达。上线前用自己的邮箱完成一次真实注册，并检查垃圾邮件；不要把 SMTP 授权码或邮件中的验证链接发到聊天、日志或 Git。
...
在题目“概览”的作者旁点击“查看联系方式”，或在“用户管理”选择账号后展开联系方式，可查看对方填写的 QQ、邮箱、姓名和认证标识，方便沟通。此操作要求人工账号拥有 `user.permission.manage`；普通审题权限不包含读取联系资料。切换为普通用户后按目标用户权限检查，不能借原管理员身份继续查看。没有填写的字段显示“未填写”，不从公开昵称猜测个人信息。
...
历史资料中的编号不一定等于数据库内部 ID。迁移必须先核对来源映射，保留完整意见和原始结论；缺少的难度、质量、思维和代码评分必须保持缺失，不能填默认分数。迁移前备份，在隔离数据库验证后再应用，具体步骤见[导入历史审核记录](history-review-import.md)。

题目创建和编辑页提供“允许 AI 审题”开关，投稿人拥有编辑权限时可修改自己题目的开关，拥有 `problem.status.change` 权限的人工账号也可管理。新增题目默认开启；关闭不影响人工审核和已有记录，外部程序不能再领取或完成该题的任务。已审核的历史题在迁移确认后关闭，避免启用 Fermata 时重复审核。

### Fermata 模型与连接配置

在「插件」启用 Fermata 并配置服务地址、管理令牌后，打开其设置页面。这里可以修改实际模型名称、OpenAI 兼容接口基础地址、温度、模型 API 密钥，以及题库 API 地址和机器人令牌。模型服务地址填写基础路径（如 `https://example.com/v1`），服务会追加 `/chat/completions`。DeepSeek V4 自动开启深度思考并使用 max 强度，随后单独整理结构化结果。

管理令牌来自部署 Fermata 时配置的 `FERMATA_MANAGEMENT_TOKEN`，用于 Urmotiv 访问 Fermata 管理接口；题库机器人令牌来自「服务账号」，用于 Fermata 领取、续租和提交审核任务；模型 API 密钥由模型提供方签发。三者用途不同。轮换题库机器人令牌后，需要将新令牌保存到 Fermata 设置中。

密钥输入框始终不回显；留空保持原值，勾选清除后保存会停用该凭据，不回退到环境里的旧密钥。设置由独立的 Fermata 服务保存，新配置用于后续任务，在途任务使用领取时的配置。版本冲突时重新读取后再编辑。页面“已配置”只证明密钥存在，不能代替实际的上游连接与审核结果检查。
...
直接使用 root 登录后，进入「管理 → 系统 → 备份与恢复」。填写 WebDAV 地址、账号和密码，再单独设置一个至少 12 字符的备份文件密码。勾选启用，点击「验证连接并保存」；服务器会验证创建文件、移动、读取与清理，失败时不覆盖原配置。也可由部署者用 `WEBDAV_ADDR`、`WEBDAV_USER`、`WEBDAV_PASS` 预填连接，网页仍需验证才会启用。

备份写入该地址下的 `urmotiv/` 文件夹。点击「立即完整备份」手动执行，或设置 1–720 小时的自动间隔；留空就是仅手动。页面显示当前阶段、上次尝试、上次成功和失败原因。新配置及操作状态保存在持久化的 `URMOTIV_BACKUP_DIR`（Compose 默认 `/app/backups`），不要丢弃该卷。

`.urb` 文件包含 PostgreSQL 数据库、账号及密码散列、题目历史、审核、比赛、头像、插件宿主配置与尚有效的附件，使用独立密码进行 AES-256-GCM 加密与完整性校验。已删除文件和已过期临时文件不包含在内。修改文件密码只影响新备份；旧文件仍需要旧密码。部署环境文件、数据库连接、当前 WebDAV 配置以及独立 Anklang/Fermata 服务自己的数据库和环境不属于 Urmotiv 站内备份，请另行保管。

恢复时读取远端列表，选择备份，输入该备份的密码、当前 root 登录密码，并输入「恢复整个站点」。服务器先验证完整性，在临时数据库演练，再短暂暂停站点操作并上传恢复前的完整快照；这几步任何一步失败都不替换在线数据库。最终数据库替换在单个事务中完成。恢复后所有登录和邮件确认链接失效，需要使用备份中的账号密码重新登录。备份来自较新数据库版本时，须先升级程序。导入、导出或外部审题仍有活动任务时，先等待完成再恢复。

此功能要求 PostgreSQL 17、配套 `pg_dump`/`pg_restore`、数据库账号拥有创建临时数据库的权限、已配置 `URMOTIV_PLUGIN_SECRET_KEY` 和足够的临时磁盘空间。项目 API 镜像已携带备份工具。当前完整包展开上限 20 GiB；超大站点使用部署级备份。仅 root 真人直接登录可管理完整备份，普通管理员、机器人和切换用户会话不能访问。
