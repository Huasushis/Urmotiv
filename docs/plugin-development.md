...
4. 最小测试、插件包 typecheck/build 和 API/Worker 构建通过；插件进入 `createBuiltinPluginDefinitions()` 并由代码审查批准。发布前必须重新构建 API/Worker 镜像，并在全新镜像中执行插件导入/启动 smoke 检查且通过，确认目标插件能正常加载。
