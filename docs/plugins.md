...
管理员必须把 `baseUrl` 配置为本地/私有地址并显式确认 `privateContentAuthorized: true`。检索来源可选 `yuantiji`（默认，仅调用公开原题搜索）、`local`（只用 Urmotiv 自己的 Anklang 索引）或 `hybrid`（两者并行合并）；切换来源不会把两套数据库合并。两个插件密钥用途不同：`serviceToken` 认证 Urmotiv→Anklang 服务请求，`embeddingApiKey` 是 local/hybrid 所需的 OpenAI 兼容嵌入提供方写密钥（只加密保存、永不回显）。`embeddingProvider` 设置（`protocol: openai`、`baseUrl`/`model`/`dimension`）只允许 HTTPS 或仅供隔离测试的本地/私有 HTTP，拒绝账号密码/查询/片段；不读取环境变量。保存插件后连接测试会分别测试来源和嵌入接口；管理员也可点击“应用已保存设置 / 刷新状态”。更换本地嵌入模型、维度或地址会在 Anklang 内按小批次全量重建向量，状态从提供方状态的 `rebuild` 查看。设置或密钥缺失时查询按 `failureBehavior` 不可用、索引同步零请求跳过。查询最多 1–3 次（默认 2），可重试的只有网络/超时/408/429/502/503/504；401、409 和契约错误不重试。同步使用 1–30 秒（默认 10 秒）的独立上限，失败不回滚 Urmotiv 已提交的本地修订。
