...
AI 审题服务，独立保存运行设置和模型配置，不与 Urmotiv 共享数据库。Urmotiv 通过带版本号的
...
正式评分路径先让模型审题，再按 JSON Schema 转换和校验意见。准确性实验不再作为运行许可；当前只验收能正确运行、格式符合接口约定，不宣称判断准确率已达标。没有查重条目也能审题，但不能声称已经查重。
...
启用后，该插件设置卡片内显示“管理审核服务”入口。Fermata 不再作为通用系统导航的固定栏目；停用插件或缺少所需权限时隐藏入口。
...
机器人只能领取 `external_review_enabled=true` 且当前处于待审核状态的题目。这个开关是题目级策略，不会绕过服务账号的服务端权限；外部程序可以使用同一类最小权限令牌，但不能读取密钥、修改权限、删除题目或模拟登录。

真人管理员在题目“审核记录”页可以开关外部验题，草稿也可以提前设置。该操作要求对本题有 `problem.status.change` 权限；保存时检查题目版本并记录审计。关闭后，已经领取但尚未提交的任务同样不能续租或写回。人工审核及已有审核记录不受影响。

新题默认开启。迁移旧题时，应先按原始资料核实题目对应关系、导入完整审核记录，再关闭这批题目的外部验题；不得把工作簿行号当作数据库题目 ID。
