...
本地登录和外部登录是两条不同路径：开发环境可以使用已配置的 demo 账号；普通生产账号可在 `/login` 输入唯一用户名或已验证邮箱，也可按部署配置使用 CAS 或 USTC OAuth。固定 root 账号在同一登录页输入用户名 `root`，使用独立的本地密码登录；即使关闭邮箱或统一身份登录，也能管理站点，不必长期保留另一个初始化管理员账号。

普通账号忘记密码时，在登录页点击“忘记密码”，通过已验证邮箱获取找回链接。登录后可在“个人资料 → 账号安全”修改密码、更换邮箱；更换邮箱先验证新地址，再解除旧主要地址并发送通知。密码或邮箱变更后需要重新登录。已知道当前 root 密码时，也可以在此修改；root 忘记密码仍使用下面的服务器恢复命令。

统一身份启用后，已有账号可在“个人资料 → 关联登录”主动绑定或解绑 USTC 身份。用户名无需与学号一致，认证学号、姓名等单独展示；学校资料不会覆盖已有联系邮箱。解绑前必须保留其他可用登录方式。
...
- **题目管理**：`/admin/problems`，按状态、来源和导入批次筛选，跨页选择最多 200 道题，批量提交审核、确认通过/不通过或撤回为草稿。每题都重新检查权限、修订号和审核轮次，失败项单独报告。
