...
import { changeMyPassword, getAccountSecurity, requestMyEmailChange } from "../lib/api";
...
export function AccountSecurityPanel() {
  ...
  const password=useMutation({mutationFn:()=>changeMyPassword({currentPassword,newPassword}),onSuccess:()=>{
  ...
  return <section id="account-security" className="plain-panel account-security-panel" aria-label="账号安全">
    ...
    {query.isPending ? <p role="status">正在读取登录方式…</p> : query.isError ? <p role="alert">暂时无法读取账号安全设置。</p> : !query.data.canChangeCredentials ?
      ...
      {!query.data.hasPassword ? <p className="notice-line">还没有设置本地密码。如果已有验证邮箱，可以先通过<Link to="/forgot-password">找回密码</Link>设置密码。</p> : null}
      <div className="account-security-grid">
        <form className="form-grid" onSubmit={event=>{
          ...
          <h3>修改密码</h3>
          <label>当前密码<input type="password" autoComplete="current-password" value={currentPassword} onChange={event=>setCurrentPassword(event.target.value)} required maxLength={1024} /></label>
          ...
          <button className="primary-button" type="submit" disabled={!query.data.hasPassword||password.isPending}>{password.isPending?"正在修改…":"修改密码并重新登录"}</button>
